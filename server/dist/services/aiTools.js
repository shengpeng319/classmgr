...
async function resolveTargetUser(ctx, args) {
    const me = await prisma_1.prisma.user.findUnique({ where: { id: ctx.userId }, select: { name: true, username: true, familyId: true, role: true } });
    ...
    if (me?.familyId && (me.role === 'admin' || me.role === 'parent') && (kw || !args?.childName)) {
        const kids = await prisma_1.prisma.child.findMany({ where: { familyId: me.familyId, ...(kw ? { name: { contains: kw } } : {}) } });
        if (kids.length === 1) {
            return { userId: ctx.userId, userName: kids[0].name, childId: kids[0].id };
        }
        if (kids.length > 1) {
            if (!kw)
                throw new Error(`家里有多个孩子：${kids.map((c) => c.name).join('、')}，请说明给谁`);
        ...
        if (kw) {
            ...
            if (candidates.length === 1)
                return { userId: candidates[0].id, userName: candidates[0].name || candidates[0].username };
        }
    }
    if (!kw) {
...
const getPoints = {
    ...
    execute: async (ctx, args) => {
        const target = await resolveTargetUser(ctx, args);
        if (target.childId) {
            const child = await prisma_1.prisma.child.findUnique({ where: { id: target.childId }, select: { points: true } });
...
const addPoints = {
    name: 'add_points',
    description: '给指定孩子加减积分，points 为正数加分、负数减分，需说明原因',
    parameters: {
        type: 'object',
        properties: {
    ...
    needsConfirm: true,
    summarize: (args) => `调整积分：${args?.childName || '孩子'} ${args?.points > 0 ? '+' : ''}${args?.points}（${args?.reason || '无原因'}）`,
    execute: async (ctx, args) => {
        const target = await resolveTargetUser(ctx, args);
        ...
        if (!target.childId)
            throw new Error(`积分挂在孩子档案上，找不到「${target.userName}」的孩子档案`);
