...
const server = new index_js_1.Server({ name: 'classmgr-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
// ============ Schedule Handlers ============
async function handleListSchedules(args) {
    const { userId } = args;
    const now = new Date();
    const where = {
        isActive: true,
...
// ============ Course Handlers ============
async function handleListCourses(args) {
...
async function handleGetCourse(args) {
    const { id } = args;
    if (!id)
        return { success: false, error: '缺少课程ID' };
...
async function handleCreateCourse(args) {
...
async function handleUpdateCourse(args) {
    const { id, operation, userId, ...updateData } = args;
    if (!id)
        return { success: false, error: '缺少课程ID' };
    try {
...
async function handleDeleteCourse(args) {
    const { id } = args;
    if (!id)
        return { success: false, error: '缺少课程ID' };
    try {
...
async function handleAddStudentToCourse(args) {
    ...
    try {
        ...
        if (e.code === 'P2002')
            return { success: false, error: '学生已在课程中' };
        throw e;
    }
}
async function handleRemoveStudentFromCourse(args) {
    // 学生已并入 User，studentId 即 userId（兼容别名）
    const courseId = args.courseId || args.id;
    const studentId = args.studentId || args.userId;
    if (!courseId || !studentId)
...
async function handleListStudents(args) {
...
async function handleGetStudent(args) {
    const { id } = args;
    if (!id)
        return { success: false, error: '缺少学生ID' };
...
async function handleCreateStudent(args) {
    const { name, avatar, points, username, password } = args;
    if (!name)
        return { success: false, error: '缺少学生姓名' };
...
async function handleUpdateStudent(args) {
...
async function handleDeleteStudent(args) {
    const { id } = args;
    if (!id)
        return { success: false, error: '缺少学生ID' };
    try {
...
async function handleGetPoints(args) {
    const { id } = args;
    if (!id)
        return { success: false, error: '缺少学生ID' };
...
async function handleUpdatePoints(args) {
    const { id, pointsDelta } = args;
    if (!id)
        return { success: false, error: '缺少学生ID' };
    if (pointsDelta === undefined)
        return { success: false, error: '缺少pointsDelta' };
...
async function handleGetCard(args) {
    const { id } = args;
    if (!id)
        return { success: false, error: '缺少卡牌ID' };
...
async function handleCreateCard(args) {
    ...
    if (!name)
        return { success: false, error: '缺少卡牌名称' };
...
async function handleDeleteCard(args) {
    const { id } = args;
    if (!id)
        return { success: false, error: '缺少卡牌ID' };
    try {
...
async function handleListStudentCards(args) {
    // 学生已并入 User，studentId 即 userId（兼容别名）
    const studentId = args.studentId || args.userId;
    if (!studentId)
        return { success: false, error: '缺少学生ID' };
...
async function handleDrawCard(args) {
    // 学生已并入 User，studentId 即 userId（兼容别名）
    const studentId = args.studentId || args.userId;
    if (!studentId)
        return { success: false, error: '缺少学生ID' };
    ...
    if (activeCards.length === 0)
        return { success: false, error: '暂无可用卡牌' };
    const card = activeCards[Math.floor(Math.random() * activeCards.length)];
    if (card.stock === 0)
        return { success: false, error: '该卡牌已售罄' };
    ...
    if (card.stock > 0)
        await prisma.card.update({ where: { id: card.id }, data: { stock: card.stock - 1 } });
...
async function handleGetUserPoints(args) {
    const { username } = args;
    if (!username)
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (operation === 'list_schedules')
        ...
        else if (operation === 'get_schedule')
            result = await handleGetSchedule(args);
        else if (operation === 'create_schedule')
            result = await handleCreateSchedule(args);
        else if (operation === 'update_schedule')
            result = await handleUpdateSchedule(args);
        else if (operation === 'delete_schedule')
            result = await handleDeleteSchedule(args);
        else if (operation === 'list_courses')
            result = await handleListCourses(args);
        else if (operation === 'get_course')
            result = await handleGetCourse(args);
        else if (operation === 'create_course')
            result = await handleCreateCourse(args);
        else if (operation === 'update_course')
            result = await handleUpdateCourse(args);
        else if (operation === 'delete_course')
            result = await handleDeleteCourse(args);
        else if (operation === 'add_student_to_course')
            result = await handleAddStudentToCourse(args);
        else if (operation === 'remove_student_from_course')
            result = await handleRemoveStudentFromCourse(args);
        ...
        else if (operation === 'list_students')
            result = await handleListStudents(args);
        else if (operation === 'get_student')
            result = await handleGetStudent(args);
        else if (operation === 'create_student')
            result = await handleCreateStudent(args);
        else if (operation === 'update_student')
            result = await handleUpdateStudent(args);
        else if (operation === 'delete_student')
            result = await handleDeleteStudent(args);
        else if (operation === 'get_points')
            result = await handleGetPoints(args);
        else if (operation === 'update_points')
            result = await handleUpdatePoints(args);
            ...
            result = await handleDeleteCard(args);
        else if (operation === 'list_student_cards')
            result = await handleListStudentCards(args);
        else if (operation === 'draw_card')
            result = await handleDrawCard(args);
        else if (operation === 'list_users')
            result = await handleListUsers(args);
        else if (operation === 'get_user')
            result = await handleGetUser(args);
        else if (operation === 'get_user_points')
            result = await handleGetUserPoints(args);
        else if (operation === 'create_user')
            result = await handleCreateUser(args);
        else if (operation === 'update_user')
            result = await handleUpdateUser(args);
        else if (operation === 'delete_user')
            result = await handleDeleteUser(args);
        else
...
//# sourceMappingURL=classmgrMcp.js.map